...
const GitHubCard = ({ copiedStates, copyToClipboard }: GitHubCardProps) => {
    return (
        <div
            ...
            <div className="relative z-10 h-full flex flex-row">
                <div className="flex items-center space-x-3 xs:space-x-4 flex-1 w-full xs:w-auto min-w-fit">
                    <div className="w-10 h-10 xs:w-12 xs:h-12 bg-[#2A2A2A] border border-[#444444] rounded-lg flex items-center justify-center p-1.5 xs:p-2 hover:rotate-360 transition-transform duration-500 ease-in-out shrink-0">
                        <Github className="w-5 h-5 xs:w-6 xs:h-6 text-gray-400" />
                    </div>
                ...
                <div className="flex flex-col space-y-3 xs:space-y-4 w-full xs:w-auto">
                    <div className="flex items-center justify-end">
                        <button
                            ...
                            className={`w-8 h-8 xs:w-10 xs:h-10 rounded-lg flex items-center justify-center transition-all duration-300 shrink-0 ${copiedStates.github ? "bg-green-500 text-white" : "bg-[#2A2A2A] border border-[#444444] text-gray-300 hover:border-purple-500 hover:text-purple-400"}`}
                            title={
                                copiedStates.github ? "Copied!" : "Copy link"
                            }
                        >
                            {copiedStates.github ? (
