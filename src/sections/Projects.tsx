                ...
                <div className="grid grid-cols-1 xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 xs:gap-8">
                    {projects.map((project, index) => (
                        <div key={index} className="group">
                            {/* Card Container */}
                            <div
                                className="relative bg-[#1A1A1A] border border-[#333333] rounded-xl p-4 xs:p-6 flex flex-col h-full scroll-reveal"
                            ...
                            >
                                ...
                                <p className="text-gray-400 text-xs xs:text-sm mb-3 xs:mb-4 flex items-center">
                                    <Calendar className="w-3 h-3 xs:w-4 xs:h-4 mr-1 xs:mr-2" />
                                    {project.period}
                                </p>

                                {/* Project Description */}
                                <ul className="list-disc list-inside text-gray-300 mb-3 xs:mb-4 grow">
                                ...
                                <div className="flex flex-wrap gap-1.5 xs:gap-2 mb-4 xs:mb-6">
                                    {project.technologies.map(
                                        (tech, techIndex) => (
                                            <span
                                                key={techIndex}
                                                className="px-2 xs:px-3 py-1 bg-[#2A2A2A] text-gray-300 text-xs rounded-full border border-[#444444]"
                                            >
                                                {tech}
                                            </span>
                                        ),
                                ...
                                {/* Action Buttons */}
                                <div className="flex gap-2 xs:gap-3 mt-auto justify-between">
                                    {project.hasCode && (
                                        <a
                                            href="#"
                                            className="flex items-center gap-1 xs:gap-2 px-3 xs:px-4 py-2 bg-linear-to-r from-purple-500 to-blue-500 text-white rounded-lg hover:from-purple-600 hover:to-blue-600 transition-all duration-200"
                                        >
